...
<?php include("./template/header.php")?>
    ...
    if(isset($_POST['submit'])) {
        $date = $_POST['date'];
        // Convert the date into an underscored name for the new table
    $temeName = $iddb . '_' . $mid . '_' . str_replace('-', '_', $date) . '_Timing';

     // Prepare the INSERT query
$insertQuery = "INSERT INTO `$table` (`DATE`) VALUES (:date)";
...
$createTableQuery = "CREATE TABLE `$temeName` (
...
$con->exec($createTableQuery);
        ...
        if ($stmt->execute()) {
          echo"<script> window.location.href='timingsindex.php?iddb=$iddb&mid=$mid&nama=$nama';</script>";
        } else {
            echo "Failed";
        }
    }
